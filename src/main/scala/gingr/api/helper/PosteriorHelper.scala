...
object PosteriorHelper {
  ...
  def computeDistanceMapFromMeshesTotal(
    ...
    def outer(v1: DenseVector[Double], v2: DenseVector[Double]): DenseMatrix[Double] =
      v1.toDenseMatrix.t * v2.toDenseMatrix

    val pointIdPointSeq: IndexedSeq[Seq[DenseVector[Double]]] = meshes.head.pointSet.pointIds.toIndexedSeq.map { id =>
  ...
  def computeDistanceMapFromMeshesNormal(
    ...
    val variance = (pointIdPointSeq zip ref.pointSet.pointIds.toIndexedSeq).map { case (samples, pId) =>
      val m: DenseVector[Double] = samples.foldLeft(zeroVec)((acc, s) => acc + s) * (1.0 / numSamples)

      val n = if (sumNormals) {
        meshes.foldLeft(zeroVec)((acc, s) =>
          acc + s.vertexNormals.atPoint(pId).normalize.toBreezeVector
        ) * (1.0 / numSamples)
      } else {
