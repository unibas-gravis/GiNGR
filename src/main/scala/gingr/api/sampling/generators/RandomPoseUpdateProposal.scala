...
case class GaussianAxisRotationProposal[State <: GingrRegistrationState[State]](
  ...
  override def logTransitionProbability(from: State, to: State): Double = {
    if (
      to.general.modelParameters.copy(pose =
        to.general.modelParameters.pose.copy(rotation = from.general.modelParameters.pose.rotation)
      ) != from.general.modelParameters
    ) {
...
case class GaussianAxisTranslationProposal[State <: GingrRegistrationState[State]](
  ...
  override def logTransitionProbability(from: State, to: State): Double = {
    if (
      to.general.modelParameters.copy(pose =
        to.general.modelParameters.pose.copy(translation = from.general.modelParameters.pose.translation)
      ) != from.general.modelParameters
    ) {
      Double.NegativeInfinity
    } else {
      val residual =
        to.general.modelParameters.pose.translation(axis) - from.general.modelParameters.pose.translation(axis)
      perturbationDistr.logPdf(residual)
    }
  }
}
