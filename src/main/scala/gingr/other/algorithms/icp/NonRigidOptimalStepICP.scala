...
import breeze.linalg.{diag, CSCMatrix, SparseVector}
...
abstract class NonRigidOptimalStepICP(
  ...
  val lmIdsOnTemplate: Seq[PointId] =
    commonLmNames
      .map(name => templateLandmarks.find(_.id == name).get)
      .map(lm => templateMesh.pointSet.findClosestPoint(lm.point).id)
  ...
  private val defaultAlpha: Seq[Double] =
    (1 to 10).scanLeft(1)((a, _) => (a * 2)).map(_.toDouble / 2).reverse.map(_ => 1e1) // ...,8,4,2,1,0.5
  ...
  ): (Seq[Point[_3D]], Seq[Double], Double) = {
    val (corr, dist) =
      NonRigidClosestPointRegistrator.ClosestPointTriangleMesh3D.closestPointCorrespondence(template, target)
  ...
  def Iteration(
    ...
    alpha: Double,
    beta: Double
  ): (TriangleMesh[_3D], Double, IndexedSeq[Point[_3D]])
}
  ...
  private def ComputeMatrixD(points: Seq[Point[_3D]]): CSCMatrix[Double] = {
    ...
    val D: CSCMatrix[Double] = CSCMatrix
      .zeros[Double](
        locn,
        4 * locn
      ) // Reference points each entry in D is a matrix (4D) NOTE: This function assumes that all points are passed in, otherwise use ComputeMatrixDL
